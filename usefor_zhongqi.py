# coding:utf-8
# @Author     : HT
# @Time       : 2022/4/17 23:13
# @File       : usefor_zhongqi.py
# @Software   : PyCharm
...
import cv2
import numpy as np
rgb_path='../recordData/RGBD_m_dl_2/rgb'
depth_path='../recordData/RGBD_m_dl_2/depth'
...
videoWrite = cv2.VideoWriter('kaiti.mp4',fourcc,fps,size)# 根据图片的大小，创建写入对象 （文件名，支持的编码器，5帧，视频大小（图片大小））
...
for i,f in enumerate(rgb_list):
    ...
    cv2.putText(img2, 'RGB', (25, 48), cv2.FONT_HERSHEY_COMPLEX, 1.5, (0, 0, 255), 2)
    cv2.putText(img2, 'Depth', (800, 48), cv2.FONT_HERSHEY_COMPLEX, 1.5, (0, 255, 0), 2)
    # img2=np.hstack((image,image_depth))
    ...
    if i>239:
        videoWrite.write(img2)  # 将图片写入所创建的视频对象
